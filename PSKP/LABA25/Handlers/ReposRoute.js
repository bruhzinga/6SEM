...
router.get('/:id/commits/:commitId', async (req, res) => {
    try {
...
router.put('/:id/commits/:commitId', async (req, res) => {
    try {
        const repo = await prisma.repos.findUnique({ where: { id: +req.params.id } });
        if (!repo) {
            res.status(404).send('Not found');
...
router.delete('/:id/commits/:commitId', async (req, res) => {
    try {
        req.ability?.throwUnlessCan('manage', 'all');
...
//# sourceMappingURL=ReposRoute.js.map