...
--Использую оконные функции найти второго по успешности сотрудника каждого отдела
--(по сумме продаж). Вывести имя, отдел, сумму продаж.

SELECT s.name, o.region, s.sales
FROM salesreps s
JOIN offices o ON s.rep_office = o.office
WHERE s.sales = (SELECT MAX(sales) FROM salesreps s2 WHERE s2.rep_office = s.rep_office AND s2.empl_num <> s.empl_num)
ORDER BY o.region, s.sales DESC;
